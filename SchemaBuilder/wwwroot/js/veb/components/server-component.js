...
class ServerComponent {
	...
	ajax(callback) {
		let self = this;
		this.url = Vvveb.Builder.iframe.contentWindow.location;
		//console.log(this.url);
		//console.log(this.content);
		self.element.animate({opacity: 0.85}, 50);

		$.ajax({
			url: this.url + '&_component_ajax=' + this.component + '&_component_id=' + this.index,
			type: 'post',
...
//				self.element[0].click();
				...
				self.element.animate({opacity: 1}, 50);
	...
	onChange(element, property, value, input)  {
				...
				if (this.content != element[0].outerHTML) {
					this.content = element[0].outerHTML;
